# Hello there; welcome 👋🏾

[![AltSchool Badge](https://img.shields.io/badge/-Engineering-6773E5?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&link=https://altschoolafrica.com/schools/engineering)](https://altschoolafrica.com/schools/engineering)
[![Twitter Badge](https://img.shields.io/badge/-@ceejay08199-1ca0f1?style=for-the-badge&logo=twitter&logoColor=white&link=https://twitter.com/ceejay08199)](https://twitter.com/ceejay08199)

I'm a Software Engineer, Frontend Engineer, Technical Writer, and Community Engineer.  
Right now, I'm blending **web engineering**, **UI/UX**, **cloud computing**, and **AI systems** as a first-year **PhD student in Computer Science at the University of Cincinnati**.  

I also teach as a **TA for Intelligent Systems and Operating Systems**, work on Svelte/React UI projects, study AWS and Kubernetes, and write clean, accessible technical content for developers.

I create technical content on [Hashnode](https://hashnode.com/@Joseph99), [Medium](https://medium.com/@umunnajoseph70), and the [OpenReplay Blog](https://blog.openreplay.com/authors/joseph-umunna/), and build useful [open-source projects](https://github.com/joseph0819).

---

**Here's a quick summary about me**:

- Pronouns: He/him  
- Education:  
  - 🎓 **PhD Student, Computer Science — University of Cincinnati**  
  - 🎓 **AltSchool Africa — School of Software Engineering (Class of 2022)**  
- Frontend: JavaScript, React.js, Svelte, Vue.js  
- Styling: Tailwind, Bulma, Bootstrap  
- Cloud & DevOps: AWS (ECR, ECS, S3, DynamoDB), Docker, Kubernetes  
- AI & Systems: Search algorithms, perceptrons, OS concepts, concurrency, Prolog  
- Projects: TrackApp, Smart Bookshelf, PassGrades, Scissor App, Altafiber Redesign  
- Job interests: Software Engineer, Frontend/UI Engineer, AI/Cloud Engineering (Intern/Entry Level)  
- Actively seeking open-source collaborations, hackathons, internships, and research opportunities  
- Contact: **umunnajoseph70@gmail.com** •  
  [View my resume](https://drive.google.com/file/d/1lmj8qqV-1HAxiqDMSBO7jal8GCV98I_W/view?usp=drive_link)

---

| <img align="center" src="https://github-readme-stats.vercel.app/api?username=joseph0819&show_icons=true&include_all_commits=true&hide_border=true" alt="Joseph's GitHub stats" /> | <img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=joseph0819&langs_count=8&layout=compact&hide_border=true" alt="Joseph's GitHub languages" /> |
| ------------- | ------------- |


